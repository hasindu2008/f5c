...
static inline void yes_or_no(opt_t *opt, uint64_t flag, int long_idx, const char *arg, int yes_to_set) //taken from minimap2
{
    if (yes_to_set) {
        if (strcmp(arg, "yes") == 0 || strcmp(arg, "y") == 0) {
            ...
            WARNING("option '--%s' only accepts 'yes' or 'no'.", long_options[long_idx].name);
        }
    } else {
        if (strcmp(arg, "yes") == 0 || strcmp(arg, "y") == 0) {
            opt->flag &= ~flag;
...
int main(int argc, char* argv[]) {
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, &longindex)) >=
        ...
        if (c == 'r') {
            ...
            opt.min_mapq = atoi(optarg);    //check whether this is between 0 and 60
