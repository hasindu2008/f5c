...
*  `--verbose INT`:
    Verbosity level for the log messages [default value: 1].
*  `--version`:
...
* `-b FILE`:
  The file containing the alignment records sorted based on genomic coordinates in BAM format.
* `-g FILE`:
...
* `--pore STR`
   Set the pore chemistry. Specify r9 for R9.4 data, r10 for R10.4 data (from v1.2) and rna004 for RNA004 (from v1.4) [default value: r9 for FAST5, autodetected for SLOW5]. Option introduced in f5c v1.2.
*  `--slow5 FILE`:
...
* `--kmer-model FILE`:
  Custom nucleotide k-mer model file. The file should adhere to the format in [r9.4_450bps.nucleotide.6mer.template.model](https://github.com/hasindu2008/f5c/blob/master/test/r9-models/r9.4_450bps.nucleotide.6mer.template.model). The maximum supported k-mer size is 9.
* `--meth-model FILE`:
  custom methylation k-mer model file. The file should adhere to the format in [r9.4_450bps.cpg.6mer.template.model](https://github.com/hasindu2008/f5c/blob/master/test/r9-models/r9.4_450bps.cpg.6mer.template.model). The maximum supported k-mer size is 9.
* `--meth-out-version INT`:
  Format version of the output Methylation tsv file. If set to 1, the columns printed adhere to the output format of Nanopolish early versions. If set to 2, adhere to the latest nanopolish output format that additionally includes the strand column and the header num_cpgs renamed to *num_motifs*) [default value: 2]
...
`f5c meth-freq [OPTIONS] -i methcalls.tsv`
