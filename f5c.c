...
db_t* init_db(){
    ...
    db->f5=(fast5_t**)malloc(sizeof(fast5_t*) * db->capacity_bam_rec); MALLOC_CHK(db->f5);
...
int32_t load_db(core_t* core,db_t* db){
    ...
    for(i=0;i<db->n_bam_rec;i++){
        ...
        char *refseq=faidx_fetch_seq(core->fai, ref_name, ref_start_pos, ref_end_pos, &fetched_len); //error handle?
        ...
        char *fast5_path=(char *)malloc(core->readbb->get_signal_path(qname).size()+10); //is +10 needed? do errorcheck
