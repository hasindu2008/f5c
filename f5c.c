...
db_t* init_db(core_t* core) {
    ...
    db->scalings=(scalings_t*)malloc(sizeof(scalings_t)* db->capacity_bam_rec);
    MALLOC_CHK(db->scalings);    

    db->event_align_pairs = (AlignedPair**)malloc(sizeof(AlignedPair*)* db->capacity_bam_rec);
    MALLOC_CHK(db->event_align_pairs);
    db->n_event_align_pairs = (int32_t*)malloc(sizeof(int32_t)* db->capacity_bam_rec);
    MALLOC_CHK(db->n_event_align_pairs);    
 
    db->event_alignment = (event_alignment_t**)malloc(sizeof(event_alignment_t*)* db->capacity_bam_rec);
    MALLOC_CHK(db->event_alignment);    
    db->n_event_alignment = (int32_t *)malloc(sizeof(int32_t *)* db->capacity_bam_rec);
    MALLOC_CHK(db->n_event_alignment);

    db->events_per_base = (double *)malloc(sizeof(double *) * db->capacity_bam_rec);
...
void *align_pthread(void *voidargs){
    ...
    for (i = args->starti; i < args->endi; i++) {    
        db->n_event_align_pairs[i] = 
            align(db->event_align_pairs[i],db->read[i], db->read_len[i],db->et[i], core->model, db->scalings[i],
                  db->f5[i]->sample_rate);
...
void align_db(core_t* core, db_t* db){
    ...
    if(core->opt.num_thread == 1){
        ...
        for(t=0;t<num_thread;t++){
            ...
            ret=pthread_create(&tids[t],NULL,align_pthread,(void*)(&pt_args[t]));
            NEG_CHK(ret);                  
        }
...
void process_db(core_t* core, db_t* db, double realtime0) {
    ...
    for (i = 0; i < db->n_bam_rec; i++) {
        db->event_align_pairs[i] = (AlignedPair*)malloc(sizeof(AlignedPair) * db->et[i].n*2); //todo : find a good heuristic to save memory
    ...
    for (i = 0; i < db->n_bam_rec; i++) {
        ...
        if (db->n_event_align_pairs[i]>0) {
            ...
            db->event_alignment[i] =
            (event_alignment_t*)malloc(sizeof(event_alignment_t) * db->n_event_align_pairs[i]);
    ...
    if (core->opt.flag & F5C_PRINT_BANDED_ALN) {
        ...
        for (i = 0; i < db->n_bam_rec; i++) {
            printf(">%s\tN_ALGN_PAIR:%d\t{ref_os,read_pos}\n",
                   bam_get_qname(db->bam_rec[i]), (int)db->n_event_align_pairs[i]);
            AlignedPair*  event_align_pairs=db->event_align_pairs[i];
            int32_t j = 0;
            for (j = 0; j < db->n_event_align_pairs[i]; j++) {
                printf("{%d,%d}\t", event_align_pairs[j].ref_pos,event_align_pairs[j].read_pos);
            }
            printf("\n");
        }
    }  
    ...
    if (core->opt.flag & F5C_PRINT_SCALING){ 
        ...
        for (i = 0; i < db->n_bam_rec; i++) {
            printf("%s\t%.2lf\t%.2lf\t%.2lf\n",bam_get_qname(db->bam_rec[i]),
                        db->scalings[i].shift, db->scalings[i].scale, db->scalings[i].var);
        }
    }
}
