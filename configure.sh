...
hdf5header()
{
	...
	echo "Manually enter the value for HDF5_INC in include.mk"
...
#echo "HTS_INC =" > include.mk
 
echo "#autogenerated from configure.sh" > include.mk
echo "int main(){return 0;}" > test.c
 
if  ldconfig -p | grep "libhts.so" > /dev/null
then
	echo "libhts found"
	echo "HTS_LDFLAGS = -lhts" >> include.mk
else
...
then
	...
	then
		echo "HDF5_LDFLAGS = -lhdf5_serial" >> include.mk
		if [ -e /usr/include/hdf5/serial/hdf5.h ] 
		then
			echo "HDF5_INC = /usr/include/hdf5/serial/" >> include.mk
		else
			hdf5header
		fi
	elif ldconfig -p | grep "libhdf5.so" > /dev/null
	then 
		echo "HDF5_LDFLAGS = -lhdf5" >> include.mk
		if [ -e /usr/include/hdf5/hdf5.h ] 
		then
			echo "HDF5_INC = /usr/include/hdf5/" >> include.mk
		else
			hdf5header
		fi		
		echo "HDF5_INC = /usr/include/hdf5/" >>  include.mk
		...
		echo "Manually enter the value for HDF5_LDFLAGS and in HDF5_INC in include.mk"
	fi
