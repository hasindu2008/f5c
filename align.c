...
// std::vector<AlignedPair> align(char* sequence,event_table events,model_t* models, scalings_t scaling){
 ...
 size_t n_events = events.n;
  ...
  trace[1][first_trim_offset] = FROM_U;
    ...
    int capacity = 100000;
    AlignedPair* out_c=(AlignedPair*)malloc(sizeof(AlignedPair)*capacity);
    int outIndex=0;
    //<<<<<<<<<<<<<<<<New Replacement over
    ...
    while(curr_kmer_idx >= 0 && curr_event_idx >= 0) {
        ...
        out_c[outIndex].ref_pos = curr_kmer_idx;
        out_c[outIndex].read_pos = curr_event_idx;
        outIndex++;
    ...
    for (c = 0; c < outIndex/2; c++) {
      int ref_pos_temp  = out_c[c].ref_pos;
      int read_pos_temp = out_c[c].read_pos;
      out_c[c].ref_pos  = out_c[end].ref_pos;
      out_c[c].read_pos = out_c[end].read_pos;
      out_c[end].ref_pos = ref_pos_temp;
      out_c[end].read_pos = read_pos_temp; 
      end--;
    }  

    // if(outIndex>1){
    //   AlignedPair temp={out_c[0].ref_pos,out[0].read_pos};
    //   int i;
    //   for(i=0;i<outIndex-1;i++){
    //     out_c[i]={out_c[outIndex-1-i].ref_pos,out[outIndex-1-i].read_pos};
    //   }
    ...
    //>>>>>>>>>>>>>New replacement begin
    bool spanned = out_c[0].ref_pos == 0 && out_c[outIndex-1].ref_pos == n_kmers - 1;
