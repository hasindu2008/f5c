...
if [[ "${#}" -eq 0 ]]; then
    "${exepath}" call-methylation -b "${bamdir}" -g "${fadir}" -r "${fastqdir}" --secondary=yes --min-mapq=20  > ${testdir}/result.txt
    ...
    if [[ "${1}" == "valgrind" ]]; then
		...
		echo "${exepath}" call-methylation -b "${bamdir}" -g "${fadir}" -r "${fastqdir}" --secondary=yes --min-mapq=20  ">" ${testdir}/result.txt
    else
        echo "wrong option"
		exit 1
    fi
