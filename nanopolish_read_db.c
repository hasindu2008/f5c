...
//
void ReadDB::import_reads(const std::string& input_filename,
                          const std::string& out_fasta_filename) {
    // Open readers
    ...
    while ((ret = kseq_read(seq)) >= 0) {
        ...
        if (iter != m_data.end()) {
            fprintf(stderr,
                    "Error: duplicate read name %s found in fasta file\n",
                    seq->name.s);
            exit(EXIT_FAILURE);
        }
    ...
    if (ret <= -2) {
        fprintf(stderr,
                "kseq_read returned %d indicating an error with the input file "
                "%s\n",
...
//
void ReadDB::add_signal_path(const std::string& read_id,
