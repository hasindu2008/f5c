...
|8  |int   |start_kmer| on basecalled sequence (0-based; see note below)  |
|9  |int   |end_kmer| on basecalled sequence (0-based; see note below)   |
...
<img width="500" alt="image" src="https://user-images.githubusercontent.com/12987163/188426287-3da88fb7-7258-4919-94a7-29091532d91f.png">


Each dot in the top sequence in the illustration represent a k-mer and the number above each dot is the corresponding k-mer index. If the basecalled read is `ACGGTAACTATACGT` and assuming the k-mer size in the model is 6, the 0th k-mer is `ACGGTA`, 1st k-mer is `CGGTAA` ... and the 9th k-mer is `ATACGT`.
Each dot in the bottom sequence in represent a raw-signal sample and the number bewlow each dot is the corresponding signal index.
...
int main(){
    ...
    // Raw signal start index for the corresponding k-mer
    int st_raw_idx[MAX_LEN_KMER];
    // Raw signal end index for the corresponding k-mer
    int end_raw_idx[MAX_LEN_KMER];
    //intialise to -1
    for(int i=0; i<MAX_LEN_KMER; i++){
        st_raw_idx[i]=end_raw_idx[i]=-1;
    }

    int st_k = start_kmer; //if DNA, start k-kmer index is start_kmer column in paf
    int end_k = end_kmer; //if DNA, end k-kmer index is end_kmer column in paf
    int8_t rna = start_kmer > end_kmer ? 1 : 0; //if RNA start_kmer>end_kmer in paf
    if(rna){
        st_k = end_kmer; //if DNA, start k-kmer index is end_kmer column in paf
        end_k = start_kmer; //if DNA, end k-kmer index is start_kmer column in paf
    }

    int i_k = st_k; //current k-mer index
    int i_raw = start_raw; //current raw signal index

    //buffer for storing digits preceding each operation
    char buff[11];
    int i_buff=0;
    ...
    while(*ss){
        ...
        if(*ss==',' || *ss=='I' || *ss=='D'){
            if(i_buff <= 0){ //if nothing in buff
                fprintf(stderr,"Bad ss: Preceding digit missing\n");
                exit(1);
            }
            ...
            if(num < 0){
                fprintf(stderr,"Bad ss: Cannot have negative numbers\n");
                exit(1);
            }
            ...
            if(*ss=='I'){ //if an insertion, current raw signal index is incremented by num
                ...
                end_raw_idx[i_k] = i_raw;
                i_raw += num;
                st_raw_idx[i_k] = i_raw;
                i_k++;
            }
        } else {
            if(!isdigit(*ss)){
                fprintf(stderr,"Bad ss: A non-digit found when expected a digit\n");
                exit(1);
            }
            buff[i_buff++]=*ss;
        }
    ...
    if(i_raw!=end_raw){ //current raw signal index should be equal to end_raw
        fprintf(stderr,"Bad ss: Signal end mismatch\n");
        exit(1);
    }
    if(i_k!=end_k){ //current k-mer index should be equal to end_k
        fprintf(stderr,"Bad ss: Kmer end mismatch\n");
        exit(1);
    }

    for(int i=st_k; i<end_k; i++){
        if(end_raw_idx[i]==-1){
            if(st_raw_idx[i] != -1) { //if st_raw_idx[i] is -1, then end_raw_idx[i] should also be -1
                fprintf(stderr,"Bad ss: This shoud not have happened\n");
                exit(1);
            }
            printf("%d\t.\t.\n", rna ? len_kmer-i-1 : i);
        }else {
