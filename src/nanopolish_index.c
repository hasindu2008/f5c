...
static const char* INDEX_USAGE_MESSAGE =
    ...
    "Build an index mapping from basecalled reads to the signals measured by "
    "the sequencer\n"
    "f5c index is equivalent to nanopolish index by Jared Simpson\n"
    "\n"
    "  -h, --help                           display this help and exit\n"
    "  -v, --verbose                        display verbose output\n"
    "  -d, --directory                      path to the directory containing "
    "the raw ONT signal files. This option can be given multiple times.\n"
    "  -s, --sequencing-summary             the sequencing summary file from "
    "albacore, providing this option will make indexing much faster\n"
    "  -f, --summary-fofn                   file containing the paths to the "
...
void index_file_from_map(
    ReadDB& read_db, const std::string& fn,
    const std::map<std::string, std::string>& fast5_to_read_name_map) {
    //PROFILE_FUNC("index_file_from_map")

    // Check if this fast5 file is in the map
    size_t last_dir_pos = fn.find_last_of("/");
    std::string fast5_basename =
        last_dir_pos == std::string::npos ? fn : fn.substr(last_dir_pos + 1);
    ...
    if (iter != fast5_to_read_name_map.end()) {
        ...
        if (opt::verbose > 0) {
            fprintf(stderr,
                    "Could not find read %s in sequencing summary file\n",
                    fn.c_str());
        }
    }
} // process_file

void index_file_from_fast5(
    ReadDB& read_db, const std::string& fn,
    const std::map<std::string, std::string>& fast5_to_read_name_map) {
...
void index_path(
    ReadDB& read_db, const std::string& path,
    const std::map<std::string, std::string>& fast5_to_read_name_map) {
        ...
        for (const auto& fn : dir_list) {
            ...
            if (is_directory(full_fn)) {
                ...
                if (!fast5_to_read_name_map.empty()) {
                    index_file_from_map(read_db, full_fn,
                                        fast5_to_read_name_map);
                } else {
...
//
void parse_sequencing_summary(const std::string& filename,
    ...
    for (char c;
        ...
        switch (c) {
            ...
            case 'v':
                ...
                break;
            case 's':
                opt::sequencing_summary_files.push_back(arg.str());
                break;
            case 'd':
                opt::raw_file_directories.push_back(arg.str());
                break;
            case 'f':
                arg >> opt::sequencing_summary_fofn;
                break;
        }
    }
