...
inline std::vector<float> make_post_flanking(const uint32_t e_start,
    ...
    if (num_events > 1) {
        ...
        {
            uint32_t event_idx =
                e_start + (num_events - 1) * event_stride; // last event
            ...
            post_flank[num_events - 2] =
                log(TRANS_START_TO_CLIP) + // transition from pre to background state
        ...
        for (int i = num_events - 3; i >= 0; --i) {
            ...
            post_flank[i] =
                log(TRANS_CLIP_SELF) + -3.0f + // emit from background
                post_flank
                    [i +
                     1]; // this accounts for the transition from start, and to silent pre
        }
    }
    return post_flank;
}
...
inline std::vector<float> make_pre_flanking(const uint32_t e_start,
    ...
    //                log(1 - TRANS_CLIP_SELF); // transition to silent pre state
    pre_flank[1] =
        log(TRANS_START_TO_CLIP) + // transition from start to the background state
        -3.0f +                    // emit from background
        log(1 - TRANS_CLIP_SELF); // transition to silent pre state
    ...
    for (size_t i = 2; i < pre_flank.size(); ++i) {
        ...
        pre_flank[i] =
            ...
            pre_flank
                [i -
                 1]; // this accounts for the transition from the start & to the silent pre
    }
...
inline std::vector<BlockTransitions>
calculate_transitions(uint32_t num_kmers, const char* m_seq,
                      const char* m_rc_seq, event_t* event, scalings_t scaling,
                      model_t* cpgmodel, uint32_t event_start_idx,
                      uint32_t event_stop_idx, uint8_t strand,
                      int8_t event_stride, uint8_t rc, double events_per_base) {
    ...
    for (uint32_t ki = 0; ki < num_kmers; ++ki) {
        ...
        float p_mm_self =
            p_stay; // probability of observing additional events from this k-mer
        float p_mm_next = 1.0f - p_mm_self - p_mk -
                          p_mb; // normal movement from state to state
...
inline float profile_hmm_fill_generic_r9(
    const char* m_seq, const char* m_rc_seq, event_t* event, scalings_t scaling,
    model_t* cpgmodel, uint32_t event_start_idx, uint32_t event_stop_idx,
    uint8_t strand, int8_t event_stride, uint8_t rc, const uint32_t e_start,
    double events_per_base, uint32_t hmm_flags, ProfileHMMOutput& output) {
    ...
    std::vector<BlockTransitions> transitions = calculate_transitions(
        num_kmers, m_seq, m_rc_seq, event, scaling, cpgmodel, event_start_idx,
        event_stop_idx, strand, event_stride, rc, events_per_base);
    ...
    std::vector<float> pre_flank =
        make_pre_flanking(e_start, num_events, event_stride);
    std::vector<float> post_flank =
        make_post_flanking(e_start, num_events, event_stride, event_stop_idx);
    ...
    for (uint32_t row = 1; row < output.get_num_rows(); row++) {
        ...
        for (uint32_t block = 1; block < num_blocks - 1; block++) {
            ...
            float lp_emission_m = log_probability_match_r9(
                scaling, cpgmodel, event, event_idx, rank, strand, 0);
            ...
            scores.x[HMT_FROM_SOFT] =
                (kmer_idx == 0 &&
                 (event_idx == e_start || (hmm_flags & HAF_ALLOW_PRE_CLIP)))
                    ? lp_sm + pre_flank[row - 1]
                    : -INFINITY;

            output.update_cell(row, curr_block_offset + PSR9_MATCH, scores,
                               lp_emission_m);

            // state PSR9_BAD_EVENT
            scores.x[HMT_FROM_SAME_M] =
                bt.lp_mb + output.get(row - 1, curr_block_offset + PSR9_MATCH);
            ...
            output.update_cell(row, curr_block_offset + PSR9_BAD_EVENT, scores,
                               lp_emission_b);

            // state PSR9_KMER_SKIP
            scores.x[HMT_FROM_SAME_M] = -INFINITY;
            scores.x[HMT_FROM_PREV_M] =
                bt.lp_mk + output.get(row, prev_block_offset + PSR9_MATCH);
            scores.x[HMT_FROM_SAME_B] = -INFINITY;
            scores.x[HMT_FROM_PREV_B] =
                bt.lp_bk + output.get(row, prev_block_offset + PSR9_BAD_EVENT);
            scores.x[HMT_FROM_PREV_K] =
                bt.lp_kk + output.get(row, prev_block_offset + PSR9_KMER_SKIP);
            scores.x[HMT_FROM_SOFT] = -INFINITY;
            output.update_cell(row, curr_block_offset + PSR9_KMER_SKIP, scores,
                               0.0f); // no emission
            ...
            if (kmer_idx == last_kmer_idx &&
                ((hmm_flags & HAF_ALLOW_POST_CLIP) ||
                 row == last_event_row_idx)) {
                float lp1 = lp_ms +
            ...
            printf("[%d %d] start: %.2lf  pre: %.2lf fm: %.2lf\n", event_idx,
                   kmer_idx, m_s + lp_emission_m, pre_flank[row - 1],
                   output.get(row, curr_block_offset + PSR9_MATCH));
            printf("[%d %d]   end: %.2lf post: %.2lf\n", event_idx, kmer_idx,
                   lp_end, post_flank[row - 1]);
#endif

#ifdef DEBUG_FILL
            printf("Row %u block %u\n", row, block);

            printf("\tPSR9_MATCH -- Transitions: [%.3lf %.3lf %.3lf %.3lf "
                   "%.3lf] Prev: [%.2lf %.2lf %.2lf %.2lf %.2lf] out: %.2lf\n",
                   bt.lp_mm_self, bt.lp_mm_next, bt.lp_bm_self, bt.lp_bm_next,
                   bt.lp_km,
                   output.get(row - 1, prev_block_offset + PSR9_MATCH),
                   output.get(row - 1, curr_block_offset + PSR9_MATCH),
                   output.get(row - 1, prev_block_offset + PSR9_BAD_EVENT),
                   ...
                   output.get(row - 1, prev_block_offset + PSR9_KMER_SKIP),
                   output.get(row, curr_block_offset + PSR9_MATCH));
            printf("\tPSR9_BAD_EVENT -- Transitions: [%.3lf %.3lf] Prev: "
                   "[%.2lf %.2lf] out: %.2lf\n",
                   bt.lp_mb, bt.lp_bb,
                   output.get(row - 1, curr_block_offset + PSR9_MATCH),
                   output.get(row - 1, curr_block_offset + PSR9_BAD_EVENT),
            ...
            printf("\tPSR9_KMER_SKIP -- Transitions: [%.3lf %.3lf %.3lf] Prev: "
                   "[%.2lf %.2lf %.2lf] sum: %.2lf\n",
...
class ProfileHMMForwardOutputR9 {
  public:
    ...
    //
    inline void update_cell(uint32_t row, uint32_t col,
                            const HMMUpdateScores& scores, float lp_emission) {
    ...
    inline float get_end() const { return lp_end; }

    inline size_t get_num_columns() const { return p_fm->n_cols; }

    inline size_t get_num_rows() const { return p_fm->n_rows; }
...
float profile_hmm_score_r9(const char* m_seq, const char* m_rc_seq,
                           event_t* event, scalings_t scaling,
                           model_t* cpgmodel, uint32_t event_start_idx,
                           uint32_t event_stop_idx, uint8_t strand,
                           int8_t event_stride, uint8_t rc,
                           double events_per_base, uint32_t hmm_flags) {
    const uint32_t k =
        KMER_SIZE; //hardcoded had this const uint32_t k = data.pore_model->k;
    uint32_t n_kmers = strlen(m_seq) - k + 1;

    uint32_t n_states =
        PSR9_NUM_STATES * (n_kmers + 2); // + 2 for explicit terminal states
    ...
    float score = profile_hmm_fill_generic_r9(
        m_seq, m_rc_seq, event, scaling, cpgmodel, event_start_idx,
        event_stop_idx, strand, event_stride, rc, e_start, events_per_base,
        hmm_flags, output);
...
float profile_hmm_score(const char* m_seq, const char* m_rc_seq, event_t* event,
                        scalings_t scaling, model_t* cpgmodel,
                        uint32_t event_start_idx, uint32_t event_stop_idx,
                        uint8_t strand, int8_t event_stride, uint8_t rc,
                        double events_per_base, uint32_t hmm_flags) {
#ifdef INPUT_DEBUG
    ...
    fprintf(stderr,
            "event_start_idx %d, event_stop_idx %d, event_stride %d, rc %d\n",
            event_start_idx, event_stop_idx, event_stride, rc);

#endif

    return profile_hmm_score_r9(m_seq, m_rc_seq, event, scaling, cpgmodel,
                                event_start_idx, event_stop_idx, strand,
                                event_stride, rc, events_per_base, hmm_flags);
}
